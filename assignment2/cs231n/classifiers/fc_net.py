...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, x2_cache = affine_forward(X2, W2, b2)
        ...
        dx2, dw2, db2 = affine_backward(softmax_grad, x2_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params["W{}".format(str(i+1))] = weight_scale * np.random.randn(layers_dims[i],layers_dims[i+1])
            self.params["b{}".format(str(i+1))] = np.zeros((layers_dims[i+1],))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            w,b = self.params["W{}".format(str(i+1))],self.params["b{}".format(str(i+1))]
            if self.normalization:
...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param, \
    normalization, dropout, do_param):
